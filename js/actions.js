$(document).ready(function(){
	...
	*/
	...
	$(document).on("click",".ready-to-checkout", function(event) {
		event.preventDefault(); // Prevent the default action of the link
	
		// Assuming you have the product ID available
		var productId = "your_product_id_here";
	
		// AJAX request to check existing quantity
		$.ajax({
			url: "check_existing_quantity.php",
			method: "POST",
			data: { productId: productId },
			success: function(response) {
				// Parse the response to get the existing quantity
				var existingQuantity = parseInt(response);
	
				// Now you can perform any action based on the existing quantity
				if (existingQuantity > 0) {
					// Proceed with checkout
					// You can redirect to the checkout page or perform any other action
					window.location.href = "checkout.php";
				} else {
					// Show a message or take action if the quantity is not sufficient
			...
			error: function(xhr, status, error) {
				...
				alert("Error checking existing quantity");
			}
		});
	});
